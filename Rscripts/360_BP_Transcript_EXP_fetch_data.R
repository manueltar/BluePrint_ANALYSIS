...
fetch_data = function(option_list)
{
  ...
  if(dim(Intersected_RNAids_sel)[1] >0)
  {
    ...
    chr_sel<-unique(as.character(Intersected_RNAids_sel$chr))
    pos_sel<-unique(as.character(Intersected_RNAids_sel$pos))
    ...
    kousik_path<-"/lustre/scratch123/hgi/mdt2/projects/hematopoiesis/Blueprint/Analysis/kk8/Manuel_variants/transcript_expression_result"
    ...
    indexes_transcript_exp <- grep("_ENST.+\\.txt$",file_list)
    
    if(Condition_DEBUG == 1)
    {
      cat("indexes_transcript_exp\n")
      cat(str(indexes_transcript_exp))
      cat("\n")
    }
    
    file_list_transcript_exp<-file_list[indexes_transcript_exp]
    
    if(Condition_DEBUG == 1)
    {
      cat("file_list_transcript_exp\n")
      cat(str(file_list_transcript_exp))
      cat("\n")
    }
    
    indexes_rsid <- grep(rsid_sel,file_list_transcript_exp)
    ...
    string_chrom_pos<-paste(chr_sel,pos_sel,sep=':')
    
    
    indexes_chrom_pos <- grep(string_chrom_pos,file_list_transcript_exp)
    ...
    file_list_transcript_exp_rsid<-file_list_transcript_exp[indexes_rsid]
    
    if(Condition_DEBUG == 1)
    {
      cat("file_list_transcript_exp_rsid\n")
      cat(str(file_list_transcript_exp_rsid))
      cat("\n")
    }
    
    if(length(file_list_transcript_exp_rsid) ==0)
    {
      
      file_list_transcript_exp_rsid<-file_list_transcript_exp[indexes_chrom_pos]
      
      if(Condition_DEBUG == 1)
      {
        cat("file_list_transcript_exp_rsid_RESCUE\n")
        cat(str(file_list_transcript_exp_rsid))
        cat("\n")
      }
    }
    
    File_array<-unique(unlist(file_list_transcript_exp_rsid))
    ...
    ENST_array<-gsub(paste(rsid_sel,"_",sep=''),"",File_array)
    ENST_array<-gsub(paste(string_chrom_pos,"_",sep=''),"",ENST_array)
    ENST_array<-gsub("_.+$","",ENST_array)
    ...
    if(length(ENST_array) >0)
    {
      ...
      for(i in 1:length(ENST_array))
      {
        
        ENST_array_sel<-ENST_array[i]
        ...
        filename=paste(rsid_sel,ENST_array_sel,"gene_cufflinks.txt",sep='_')
        ...
        if (file.exists(filename)) {
          ...
          if(alt_sel_BP == alt_sel)
          {
            ...
            TRANSCRIPTS_table_subset_sel<-TRANSCRIPTS_table[which(TRANSCRIPTS_table$transcript_id == ENST_array_sel),]
            
            if(Condition_DEBUG == 1)
            {
              cat("TRANSCRIPTS_table_subset_sel:\n")
              cat(str(TRANSCRIPTS_table_subset_sel))
              cat("\n")
            }
            ...
            if(dim(TRANSCRIPTS_table_subset_sel)[1] >0)
            {
              BP_EXP_subset$transcript_id<-TRANSCRIPTS_table_subset_sel$transcript_id
              BP_EXP_subset$ensembl_gene_id<-TRANSCRIPTS_table_subset_sel$ensembl_gene_id
              BP_EXP_subset$HGNC<-TRANSCRIPTS_table_subset_sel$HGNC
              ...
              list_transcripts[[i]]<-BP_EXP_subset
      ...
      if(length(list_transcripts) >0)
      {
        BP_genes_DEF = as.data.frame(data.table::rbindlist(list_transcripts, fill=T), stringsAsFactors=F)
        ...
        saveRDS(file="BP_Transcript_EXP_for_LM.rds", BP_genes_DEF)
        
        
      }# length(list_transcripts) >0
